...
export async function getBookings(filter, sortBy, currentPage) {
  let query = supabase
    .from("bookings")
    .select(
      "id,created_at,startDate,endDate,numGuests,numNights, totalPrice,status, cabins!bookings_cabinId_fkey(name), guests(fullName,email)",
      { count: "exact" }
    );
...
export async function getBooking(id) {
  const { data, error } = await supabase
    .from("bookings")
    .select("*, cabins!bookings_cabinId_fkey(*), guests(*)")
    .eq("id", id)
    .single();
